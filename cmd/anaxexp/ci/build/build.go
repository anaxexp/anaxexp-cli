...
const DockerfileTpl = `ARG ANAXEXP_BASE_IMAGE
...
var Cmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if config.DataContainer != "" {
			...
			to := fmt.Sprintf("/tmp/anaxexp-build-%s", config.DataContainer)
		...
		var context string
		if config.DataContainer != "" {
			context = fmt.Sprintf("/tmp/anaxexp-build-%s", config.DataContainer)
		} else {
		...
		for _, service := range services {
			...
			buildArgs["ANAXEXP_BASE_IMAGE"] = service.Image
